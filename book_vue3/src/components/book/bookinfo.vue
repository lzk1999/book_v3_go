      ...
      <div id="box" v-loading="!bookboxshow"
            ...
            <div class="bookdo">
              <el-row>
                <el-button color="#626aef" plain size="large" auto-insert-space v-if="joinShelfBtnShow"
                  @click="joinbookshelf(book.catalogue_href_list[0].substring(book.catalogue_href_list[0].lastIndexOf('/')+1))">加入书架</el-button>
        ...
        <div class="book-catalogue-list">
          <cataloguebox v-for="(catalogue_text, index) in book.catalogue_text_list" :key="index"
            :id="'catalogue_' + book.catalogue_href_list[index].substring(book.catalogue_href_list[index].lastIndexOf('/')+1)" :catalogue_text="catalogue_text" :index="index"
