        ...
        <div class="catalogue-container">
          ...
          <div class="catalogue-list">
            <div v-for="(catalogue, index) in catalogue_list" :key="index + 1"
                  @click="jumpcontent(catalogue_href[index].substring(catalogue_href[index].lastIndexOf('/')+1))"
              :id="'catalogue_' + catalogue_href[index].substring(catalogue_href[index].lastIndexOf('/')+1)">{{ catalogue }}</div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    initcatalogue(count) {
      axios
        ...
        .then((res) => {
          ...
          this.catalogue_href = res.data.catalogue_href_list;
          setTimeout(() => {
    ...
    jumpcontent(value) {
      let href = this.catalogue_href
      let lastnum = this.catalogue_href[this.catalogue_href.length-1].substring(this.catalogue_href[this.catalogue_href.length-1].lastIndexOf('/')+1)
      if (value < href[0].substring(href[0].lastIndexOf('/')+1)){
        ElNotification({ message: '当前已经是第一章！', type: 'info', duration: 2500 });
        return
      }else if(value > lastnum){
        ElNotification({ message: '当前已经是末尾章节！', type: 'info', duration: 2500 });
        return
      }else {
    ...
    setContent(content, catalogue_name) {
      $('.content').html('<div class="content-now"><div class="text-2xl p-3 tracking-widest text-center">' + catalogue_name + '</div><div style="white-space: break-spaces;" class="py-3 px-3 sm:px-9">' + content + '</div></div>');
    },
    preAppendContent(content, catalogue_name) {
      $('.content').append('<div class="content-prenow"><div class="text-2xl p-3 tracking-widest text-center">' + catalogue_name + '</div><div style="white-space: break-spaces;" class="py-3 px-3 sm:px-9">' + content + '</div></div>')
    },
    setNowContent() {
